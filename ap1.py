...
tweets_df['statment']=setm
postive = st.checkbox('show me postive tweet')
if postive:
   st.dataframe( tweets_df[tweets_df['statment']=='positive']['Tweet'].head())
negative = st.checkbox('show me negative tweet')
if negative:
    st.dataframe(tweets_df[tweets_df['statment']=='negative']['Tweet'].head())
#setm
#setm
...
names= tweets_df['Username'].tolist()
...
for i in range(len(c)):
  try:
   g=Genderize().get([c[i]])
   ge.append(g[0]['gender'])
  except:
    i=i+1
import matplotlib.pyplot as pyplot
import plotly.express as px

res = [i for i in ge if i is not None]
print(res)
from collections import Counter
print(Counter(res))
counts=Counter(res)
